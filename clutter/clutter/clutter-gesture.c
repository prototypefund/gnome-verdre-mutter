 ...
 *
 * - RECOGNIZED: The gesture was sucessfully recognized and has ended, after
 *   entering this state, the gesture will implicitly be moved to WAITING and
 *   no more points_*() vfuncs get emitted.
 *
 * - CANCELLED: The gesture was either not started at all because preconditions
 *   were not fulfilled or it was cancelled by the implementation. After
 *   entering this state, the gesture will implicitly be moved to WAITING and
 *   no more points_*() vfuncs get emitted.
 *
 ...
 * the RECOGNIZED or the CANCELLED state after points have been added. Do never
 * leave the gesture in the POSSIBLE state.
 *
           ...
           unsigned int         recursion_depth)
{
  ...
  switch (priv->state)
    ...
    case CLUTTER_GESTURE_STATE_RECOGNIZE_PENDING:
      g_assert ((new_state == priv->pending_state && priv->inhibited_count == 0) ||
                new_state == CLUTTER_GESTURE_STATE_CANCELLED);
      break;
    case CLUTTER_GESTURE_STATE_RECOGNIZING:
      g_assert (new_state == CLUTTER_GESTURE_STATE_RECOGNIZING ||
                ...
                new_state == CLUTTER_GESTURE_STATE_CANCELLED);
      break;
    case CLUTTER_GESTURE_STATE_RECOGNIZED:
      ...
      if (new_state == CLUTTER_GESTURE_STATE_POSSIBLE)
        {
          if (!gesture_may_start (self))
            {
              /* No points were added yet, so let's pretend nothing happened */
              return;
            }
      ...
      for (i = 0; i < len; i++)
        {
          ...
          /* Only one influencing op can happen FIXME: not sure this is needed still */
      ...
      for (i = 0; i < len; i++)
        {
          ...
          other_priv = clutter_gesture_get_instance_private (other_gesture);
          if (other_priv->state == CLUTTER_GESTURE_STATE_RECOGNIZED ||
              other_priv->state == CLUTTER_GESTURE_STATE_CANCELLED)
            set_state (other_gesture, CLUTTER_GESTURE_STATE_WAITING, recursion_depth);
        }

      g_assert (priv->state == CLUTTER_GESTURE_STATE_RECOGNIZING ||
      ...
      for (i = 0; i < len; i++)
        {
          ...
          if (uninhibit_gesture (other_gesture)) // RECOGNIZE_PENDING -> RECOGNIZING (CANCEL us)
            {
              if (other_priv->pending_state)
                {
                  set_state (other_gesture, other_priv->pending_state, recursion_depth);
                  maybe_influence_other_gestures (other_gesture, recursion_depth + 1);
                }
            }
          else
                         ...
                         unsigned int         recursion_depth)
{
  ...
  if (priv->state == CLUTTER_GESTURE_STATE_RECOGNIZED ||
      (priv->state == CLUTTER_GESTURE_STATE_CANCELLED &&
       priv->points->len == 0))
    set_state (self, CLUTTER_GESTURE_STATE_WAITING, 0);
}
  ...
  if (priv->state == CLUTTER_GESTURE_STATE_CANCELLED)
    {
      ...
      if (event_type == CLUTTER_BUTTON_RELEASE ||
          ...
          event_type == CLUTTER_TOUCH_CANCEL)
        {
          g_array_remove_index (priv->points, point - (GesturePointPrivate *) priv->points->data);
...
clutter_gesture_class_init (ClutterGestureClass *klass)
{
   ...
   * active and move to RECOGNIZING. It's purpose is to allow the
  ...
  if ((priv->state == CLUTTER_GESTURE_STATE_WAITING &&
      ...
      (priv->state == CLUTTER_GESTURE_STATE_POSSIBLE &&
       (state == CLUTTER_GESTURE_STATE_RECOGNIZING ||
        state == CLUTTER_GESTURE_STATE_RECOGNIZED ||
