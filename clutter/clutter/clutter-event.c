...
gboolean
_clutter_event_process_early_filters (ClutterEvent *event)
{
  ClutterMainContext *context = _clutter_context_get_default ();
  GList *l, *next;
  ...
  for (l = context->event_early_filters; l; l = next)
    {
...
gboolean
_clutter_event_process_late_filters (ClutterEvent *event)
{
  ClutterMainContext *context = _clutter_context_get_default ();
  GList *l, *next;
  ...
  for (l = context->event_late_filters; l; l = next)
    {
      ClutterEventFilter *event_filter = l->data;
...
/**
 * clutter_event_add_early_filter:
 ...
 */
guint
clutter_event_add_early_filter (ClutterStage          *stage,
                          ClutterEventFilterFunc func,
                          GDestroyNotify         notify,
                          gpointer               user_data)
{
  ...
  context->event_early_filters = g_list_append (context->event_early_filters, event_filter);

  return event_filter->id;
}

/**
 * clutter_event_add_late_filter:
 * @stage: (allow-none): The #ClutterStage to capture events for
 * @func: The callback function which will be passed all events.
 * @notify: A #GDestroyNotify
 * @user_data: A data pointer to pass to the function.
 ...
 */
guint
clutter_event_add_late_filter (ClutterStage          *stage,
                          ClutterEventFilterFunc func,
                          GDestroyNotify         notify,
                          gpointer               user_data)
{
  ...
  /* The event filters are kept in order from least recently added to
   * most recently added so we must add it to the end */
  context->event_late_filters = g_list_append (context->event_late_filters, event_filter);

  return event_filter->id;
}

/**
 * clutter_event_remove_early_filter:
 ...
 */
void
clutter_event_remove_early_filter (guint id)
{
  ClutterMainContext *context = _clutter_context_get_default ();
  GList *l;

  for (l = context->event_early_filters; l; l = l->next)
    {
      ClutterEventFilter *event_filter = l->data;
      ...
      if (event_filter->id == id)
        {
          ...
          context->event_early_filters = g_list_delete_link (context->event_early_filters, l);
...
/**
 * clutter_event_remove_late_filter:
 * @id: The ID of the event filter, as returned from clutter_event_add_filter()
 *
 * Removes an event filter that was previously added with
 * clutter_event_add_filter().
 ...
 */
void
clutter_event_remove_late_filter (guint id)
{
  ClutterMainContext *context = _clutter_context_get_default ();
  GList *l;

  for (l = context->event_late_filters; l; l = l->next)
    {
      ClutterEventFilter *event_filter = l->data;
      ...
      if (event_filter->id == id)
        {
          ...
          context->event_late_filters = g_list_delete_link (context->event_late_filters, l);
          g_slice_free (ClutterEventFilter, event_filter);
          return;
        }
    }
