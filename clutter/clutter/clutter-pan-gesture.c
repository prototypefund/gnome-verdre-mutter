  ...
  for (i = 0; i < n_points; i++)
    {
      ...
      float point_d_x = point->move_coords.x - point->previous_coords.x;
      float point_d_y = point->move_coords.y - point->previous_coords.y;
...
//g_warning("ENDED using point %d now, total %d", point->index, total_n_points);
  ...
  if (clutter_gesture_get_state (gesture) == CLUTTER_GESTURE_STATE_RECOGNIZING)
    {
      graphene_vec2_t velocity;
      calculate_velocity (self, points[0]->event_time, &velocity);
               ...
               ClutterGestureState  new_state)
{
  ...
  if (old_state != CLUTTER_GESTURE_STATE_RECOGNIZING &&
    ...
    {
      const ClutterGesturePoint *points = clutter_gesture_get_points (gesture, NULL);

      g_signal_emit (self,
                     obj_signals[PAN_BEGIN], 0,
                     points ? points[0].begin_coords.x : -1,
                     points ? points[0].begin_coords.y : -1);
    }
